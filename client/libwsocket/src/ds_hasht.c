...
static void release_rwlock(void *rwlock)
{
...
hashval_t hasht_get_hashval(hasht_t *h, const hashkey_t *key, void *data)
{
	struct hasht_st *self=h;

	return self->hash_func(get_key_ptr(key, data), key->len);
}
