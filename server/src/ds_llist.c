 ...
 */
int llist_prepend_unlocked(llist_t *ll, void *data)
{
	llist_node_t *lnode;
	if (ll->nr_nodes < ll->volume) {
		lnode = llist_new_node();
		...
		lnode->ptr = data;
		lnode->prev = &ll->dumb;
		lnode->next = ll->dumb.next;
		ll->dumb.next->prev = lnode;
		ll->dumb.next = lnode;
...
int llist_prepend(llist_t *ll, void *data)
{
	int ret;
	...
	ret = llist_prepend_unlocked(ll, data);
...
int llist_prepend_nb(llist_t *ll, void *data)
{
	int ret;
