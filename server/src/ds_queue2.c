...
{
	struct queue_st *q;
	q = malloc(sizeof(*q) + max*sizeof(intptr_t));
	if (q == NULL) {
		return NULL;
	}
	q->max = max;
...
static void cleanup_unlock(void *args)
{
	pthread_mutex_t *mut = args;
	pthread_mutex_unlock(mut);
}

int queue_enqueue(queue_t *ptr, intptr_t data)
{
...
int queue_dequeue(queue_t *ptr, intptr_t *data)
{
	struct queue_st *q=ptr;
...
int queue_enqueue_nb(queue_t *ptr, intptr_t data)
{
	struct queue_st *q=ptr;
...
int queue_dequeue_nb(queue_t *ptr, intptr_t *data)
{
	struct queue_st *q=ptr;
...
cJSON *queue_serialize(queue_t *ptr)
{
	struct queue_st *q=ptr;
    cJSON *result;
